...
Download the latest APK from [GitHub Releases](https://github.com/Thewanwan/new-x-downloader/releases).
...
6. Copy `auth_token=xxx` and `ct0=xxx` values

### 2. Configure the App

1. Open X Downloader
2. Tap the **Settings** icon (gear) in the top right
3. Paste `auth_token` and `ct0` in the Cookie fields
...
4. Choose: **Incremental** (new content only) or **Full** (re-download everything)
...
从 [GitHub Releases](https://github.com/Thewanwan/new-x-downloader/releases) 下载最新 APK。
...
6. 复制 `auth_token` 和 `ct0` 的值

### 2. 配置应用

1. 打开 X Downloader
2. 点击右上角的 **设置** 图标（齿轮）
3. 在 Cookie 字段填入 `auth_token` 和 `ct0`
4. 返回主页
...
X Downloader は、Twitter/X のメディアコンテンツ（画像、動画、GIF）をダウンロードするための Android アプリケーションです。複数ユーザーのサポート、インクリメンタルダウンロード、ダウンロード履歴のローカルデータベース管理に対応しています。
...
[GitHub Releases](https://github.com/Thewanwan/new-x-downloader/releases) から最新 APK をダウンロードしてください。
...
6. `auth_token` と `ct0` の値をコピー

### 2. アプリを設定する

1. X Downloader を開く
2. 右上の **設定** アイコン（歯車）をタップ
3. Cookie フィールドに `auth_token` と `ct0` を入力
4. ホーム画面に戻る
...
X Downloader는 Twitter/X의 미디어 콘텐츠(이미지, 비디오, GIF)를 다운로드하기 위한 Android 애플리케이션입니다. 여러 사용자 계정 지원, 점진적 다운로드, 다운로드 기록을 추적하는 로컬 데이터베이스를 유지합니다.
...
[GitHub Releases](https://github.com/Thewanwan/new-x-downloader/releases)에서 최신 APK를 다운로드하세요.
...
6. `auth_token`과 `ct0` 값 복사

### 2. 앱 설정하기

1. X Downloader 열기
2. 오른쪽 상단의 **설정** 아이콘(톱니바퀴) 탭
3. Cookie 필드에 `auth_token`과 `ct0` 입력
4. 홈 화면으로 돌아가기
